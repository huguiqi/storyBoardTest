...
//*********必须要将设置的往返时间格式化到这个日历中，不然时间就不对了
- (void)setStartDate:(NSDate *)aStartDate
{
    NSDate *startDate = [self clampDate:aStartDate toComponents:NSDayCalendarUnit|NSMonthCalendarUnit|NSYearCalendarUnit];
    _startDate = startDate;
}

- (void)setEndDate:(NSDate *)aEndDate
{
    NSDate *backDate = [self clampDate:aEndDate toComponents:NSDayCalendarUnit|NSMonthCalendarUnit|NSYearCalendarUnit];
    _endDate = backDate;
}
